...
namespace SightstealerColony
{
    ...
    public static class VeiledOfferingUtility
    {
        ...
        private static void ApplyFailureEffects(List<Pawn> participants, bool severe)
        {
            foreach (Pawn participant in participants)
            {
                ...
                SightstealerUtility.AddHediff(participant, "PsychicShock");
    ...
    {
        public CompProperties_SSTeleport() { compClass = typeof(CompAbilityEffect_SSTeleport); destination = AbilityEffectDestination.Caster; requiresLineOfSight = true; range = 24.9f; }
    }
    public class CompAbilityEffect_SSTeleport : CompAbilityEffect_Teleport
    {
        ...
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            if (pawn == null || pawn.Map == null || !destination.IsValid) return;
            SkipUtility.SkipTo(pawn, destination.Cell, pawn.Map);
            if (Props.destClamorType != null)
                GenClamor.DoClamor(pawn, destination.Cell, Props.destClamorRadius, Props.destClamorType);
